...
INCEPTION_V3_PATH = os.environ.get(
    "INCEPTION_V3_PATH",
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "ckpts", "inception_v3", "classify_image_graph_def.pb"),
)
    ...
    def read_activations(self, npz_path: Union[str, np.ndarray]) -> Tuple[np.ndarray, np.ndarray]:
        if isinstance(npz_path, str):
            with np.load(npz_path) as obj:
                keys = set(obj.files)
            # stats-only npz (mu/sigma without arr_0): skip Inception forward.
            if "mu" in keys and "sigma" in keys and "arr_0" not in keys:
                dummy = np.zeros((0, 1), dtype=np.float32)
                return dummy, dummy
